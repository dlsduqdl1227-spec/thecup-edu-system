...
test("ships the branded reservation portal and preserves the monochrome admin application", async () => {
  ...
  assert.doesNotMatch(app, /CSV|추가 등록 매출|추가 등록 지출|selectedMonth\.source/);
  assert.match(app, /월별 상세/);
  assert.match(app, /role="tablist"/);
  assert.match(app, /monthly-detail-content/);
  assert.match(app, /won\.format\(selectedMonth\.revenue\)/);
  assert.match(app, /won\.format\(selectedMonth\.expense\)/);
  assert.match(app, /won\.format\(selectedMonth\.profit\)/);
  assert.match(app, /월별 상세 내역/);
  assert.match(app, /월 단위 합산 내역/);
  assert.match(app, /selectedMonthDetailCount\}건/);
  assert.match(app, /selectedMonthHasBaseExpense/);
  ...
  assert.match(styles, /\.monthly-expense-entry/);
